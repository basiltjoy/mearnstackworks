import React from "react";
import { Carousel,  Container } from "react-bootstrap";
import Navbar from 'react-bootstrap/Navbar'
import "./Home.css";

function Header() {
  /*  const name = "Restaurant"        */
  return (
    <div>
    
    <Navbar bg="dark" fixed="top">
    <Container> 
      <Navbar.Brand><h1 className="caravan">Caravan</h1></Navbar.Brand>
    </Container>
  </Navbar>


    <Carousel className="carousel">
        <Carousel.Item>
          <img
            className="d-block w-100 " 
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTuCe2_qZneiTUkw64y19r8um5CjRiBS6KavQ&usqp=CAU"
            alt="First slide"
          />
          <Carousel.Caption>
            <p> </p>
          </Carousel.Caption>
        </Carousel.Item>
        <Carousel.Item>
          <img
            className="d-block w-100"
            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRjV4kzQOS61w4zCPZjTNEoF4AY5q1ng8B1Ag&usqp=CAU  "
            alt="Second slide"
          />

          <Carousel.Caption>
            <p> </p>
          </Carousel.Caption>
        </Carousel.Item>
        <Carousel.Item>
          <img
            className="d-block w-100"
            src="data:image/png;base64,  "
            alt="Third slide"
          />
          <Carousel.Caption>
            <p> </p>
          </Carousel.Caption>
        </Carousel.Item>
    </Carousel>    
    </div>
  );
}

export default Header;
